...
function TaskManager() {
    ...
    const handleTask = () => {
        ...
        } else if (updateTask === null && input && date) {
            handleAddTask();
        }
        setInput('');
        setDate('');
            ...
            <div className={styles.taskList}>
                {tasks.map((item) => (
                    <div key={item._id} className={`${styles.taskItem} ${item.isDone ? styles.done : ''}`}>
                        <span className={styles.taskName}>{item.taskName} - {new Date(item.date).toLocaleDateString()}</span>
